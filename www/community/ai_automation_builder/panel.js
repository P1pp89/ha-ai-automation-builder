...
class AIAutomationBuilderPanel extends LitElement {
  ...
  exportYAML() {
    if (!this.yamlOutput) {
      this._showNotification(this.t.error + ' - Nulla da esportare', 'error');
      return;
    }

    navigator.clipboard.writeText(this.yamlOutput).then(() => {
      this._showNotification('📋 Copiato negli appunti!', 'success');
      setTimeout(() => {
        window.location.href = '/config/automation/dashboard';
      }, 1000);
    }).catch(() => {
      this._showNotification(this.t.error + ' - Copia fallita', 'error');
    });
  }
